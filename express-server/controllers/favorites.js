...
const favoriteMemes = async (req, res) => {
  Collection.find({ name: req.params.name }) // empty filter object to return all the collections
    .exec((err, collection) => {
      // callback with error object and collection(s)
      if (!collection) {
        // if no collection
        return res
...
const addToFavoritesCollection = async (req, res) => {
  ...
  Collection.updateOne(
    { account: req.body.account}, // find collection
    { $push: { images: image } }, // push new subdocument into images array
